Chạy file
python -m graphsage.unsupervised_train --train_prefix C:/nam/work/temp/data/example_data/ppi --model graphsage_mean --max_total_steps 1000 --validate_iter 10
# chạy embedding khi không có feature của node.
python -m graphsage unsupervised_train.py --train_prefix C:/nam/work/facebook/face --model graphsage_mean --max_total_steps 1000 --validate_iter 10 --identity_dim 1

# chạy model đã được trained, để embed cho node mới. (mặc dù phải train lại toàn bộ Graph, nhưng tốc độ sẽ nhanh hơn so với node2vec khi gặp node mới, do model k phải train nữa, mà chỉ sử dụng những parameter đã được learn)
python -m graphsage unsupervised_train.py --train_prefix C:/nam/work/facebook/face --model graphsage_mean --max_total_steps 1000 --validate_iter 10 --identity_dim 1 --train_inference test
