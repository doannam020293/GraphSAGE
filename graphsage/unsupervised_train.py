...
#core params..
flags.DEFINE_string('train_inference', 'train', 'model chay dưới dạng train hay inferece')
...
def save_val_embeddings(sess, model, minibatch_iter, size, out_dir, mod=""):
    '''
    function tạo embedding dựa vào model đã  learn chạy. 
    :param sess: 
    :param model: 
    :param minibatch_iter: 
    :param size: 
    :param out_dir: 
    :param mod: 
    :return: 
...
def train(train_data, test_data=None):
    ...
    # kiểm tra xem model cần train hay inference (load trained model)
    if FLAGS.train_inference =='train':
        ...
        for epoch in range(FLAGS.epochs):
            ...
            if total_steps > FLAGS.max_total_steps:
                    break
        saver.save(sess, os.path.join(log_dir(), "final model","model.ckpt"), global_step=total_steps)
        print("Optimization Finished!")
    else:
        print('load trained mode')
        file = os.path.join(log_dir(), "final model")
